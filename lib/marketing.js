const sendMessage= (text, array , client,id)=>{
    let i = 0; 
 ...
 const TextSender = (client, text, id)=>{
 try {
       let users = 100
       let array = []
       const interval = setInterval(async()=>{
       ...
       if(result){
        ...
        client.sendMessage(m.sender, {text:`Loading: *${Math.floor((array.length/users)*100)}%* . . .`})
        console.log(array.length, 'array length')
        if(array.length >= users)
        {
        clearInterval(interval)
        sendMessage(text, array , client,id)
        }
       }
       },500)
 } catch (error) {
